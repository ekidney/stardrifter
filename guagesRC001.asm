...
XBARCHARACTERS
    BYTE $20,$20,$74,$75,$61,$f6,$ea,$a0,$A0  ; $20 A0 65 e7;
    ;     0   1   2   3   4   5   6   7   8
        byte 0
...
printr  LDX REMAIN              ;get remainder
        ...
        STA $11A2,X             ;store final position as addition to starting point of 
        RTS
...
barloop LDA #$A0        ;load a block
        CPY #$00
        BEQ noblock
        STA $11A2,X     ;store block on screen location
noblock LDA BARCOLOR    ;load current color
        STA $95A2,X     ;store color
        INX
        STA $95A2,X     ;debug by printing colour on next location
        DEX             ;debug cont.
